...
class Player(pg.sprite.Sprite):
    ...
    def load_images(self):
        self.standing_frames = [
            self.game.spritesheet.get_image(299, 123, 23, 24),
        ...
        self.walk_frames_r = [
            self.game.spritesheet.get_image(274, 136, 23, 24),
...
class Platform(pg.sprite.Sprite):
    def __init__(self, game, x, y, ids):
        ...
        self.id = ids
        self.images = [
            pg.image.load(path.join(self.game.img_dir, "static1.png")),
            pg.image.load(path.join(self.game.img_dir, "static2.png")),
        ]  # , self.game.spritesheet.get_platform(0,96,201,100)]
        moving_images = [pg.image.load(path.join(self.game.img_dir, "move.png"))]
        if self.id == 3:
            self.image = pg.image.load(path.join(self.game.img_dir, "static1.png"))
        ...
        if MOVE_PLAT_CHANCE > randrange(0, 25):
            ...
            self.image = choice(
                [pg.image.load(path.join(self.game.img_dir, "beeplt.png"))]
            )
            self.image = pg.transform.scale(self.image, (380 // 2 - 20, 94 // 2 - 20))
            self.type = "bee"
            self.id = 1
            ...
            if self.image == self.images[0]:
                self.image = pg.transform.scale(
                    self.image, (380 // 2 - 20, 94 // 2 - 20)
                )
                self.id = 1
            else:
                self.image = pg.transform.scale(
                    self.image, (216 // 2 - 20, 94 // 2 - 20)
                )
            self.image.set_colorkey(BLACK)
...
class Powerup(pg.sprite.Sprite):
    def __init__(self, game, plat):  # WORK ON ADDING SHIELDS
        ...
        self.load_images()
        self.plat = plat
        self.type = choice(["boost", "jump", "jetpack", "health", "machinegun"])
        if self.type == "boost":
    ...
    def load_images(self):
        self.fan_frames = [
            self.game.spritesheet.get_image(0, 238, 23, 8),
        ...
        for frame in self.fan_frames:
            frame.set_colorkey(BLACK)
        self.jump_frames = [
            self.game.jump_sprites.get_image(0, 10, 25, 8),
...
class Bee(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.left < 0:
            self.vx = 3
        self.dt += 60
        if self.game.bee_fast_shoot == True:
            self.shoot_timer += 10
            if self.dt > 500:
                self.shoot()
                self.dt = 0
            if self.shoot_timer > 2000:
                self.shoot_timer = 0
                self.game.bee_fast_shoot = False
        if self.dt > 3500 and self.game.bee_fast_shoot == False:
            self.shoot()
            self.dt = 0
...
class Misc(pg.sprite.Sprite):
    def __init__(self, game, plat):
        ...
        self.image = choice(
            [
                self.game.misc_sprites.get_tree(217, 0, 61, 88),
                self.game.misc_sprites.get_tree(77, 109, 70, 88),
                self.game.misc_sprites.get_tree(0, 0, 85, 107),
                self.game.misc_sprites.get_tree(0, 109, 75, 104),
            ]
        )
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
        self.rect.centerx = randrange(
            self.plat.rect.left + 30, self.plat.rect.right - 30
        )
        self.rect.bottom = self.plat.rect.top
...
class Health(pg.sprite.Sprite):
    def __init__(self, game):
        ...
        self.image = pg.image.load(
            path.join(self.img_dir, "health.png")
        ).convert_alpha()
...
class newBack(pg.sprite.Sprite):
    def __init__(self, game, y):
        ...
        self.image = choice(
            [
                pg.image.load(path.join(self.img_dir, "back1.png")),
                pg.image.load(path.join(self.img_dir, "back2.png")),
                pg.image.load(path.join(self.img_dir, "back3.png")),
                pg.image.load(path.join(self.img_dir, "back4.png")),
                pg.image.load(path.join(self.img_dir, "back5.png")),
            ]
        )
        self.rect = self.image.get_rect()
        self.rect.x = 0
...
class Jetpack(pg.sprite.Sprite):
    def __init__(self, game):
        ...
        self.type = "jet"
        self.image = pg.image.load(
            path.join(self.game.img_dir, "rocket.png")
        ).convert_alpha()
...
class Peashooter_head(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        self.rect.centerx = self.shooter.rect.centerx
        self.current_clock += self.clock_DT
        if self.current_clock > 3000:
...
class Peashooter_bullet(pg.sprite.Sprite):
    ...
    def update(self):
        self.shoot()
        if (self.rect.x > WIDTH or self.rect.x < 10) or (
            self.rect.y > HEIGHT or self.rect.y < 10
        ):
            self.kill()
