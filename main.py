...
#CREATE ACHIEVMENTS _____________________________
#CREATE MAIN MENU WITH TOGGLE OPTIONS
#CREATE STATISTICS
...
class Game:
    ...
    def update(self):
        ...
        for back in self.backgrounds:
            main = list(back.rect)
            first = main[1]
            #print(len(self.backgrounds) , ' ', first)
            if first > -60 and first < -40: #or len(self.backgrounds) == 1:
                
                if len(self.backgrounds) <= 1 and self.player.vel.y <= 0:
                    print('new background init')
        ...
        if self.jetpackLaunch == True:    # while jetpack is fireing
            ...
            self.particles.append([[self.player.rect.centerx + 5, self.player.rect.bottom + 20], [random.randint(0,20) / 10 -1, 2], random.randint(5,8)])
        ...
        for particle in self.particles:
            ...
            particle[2] -= 0.1
            particle[1][1] += 0.3
            pygame.draw.circle(self.MAINWINDOW, choice([RED, ORANGE, YORANGE,YELLOW]), [int(particle[0][0]), int(particle[0][1])], int(particle[2]) )
            if particle[2] <= 0:
                self.particles.remove(particle)      
        pg.display.flip() 
                      
        self.eventlist = ['F: ' + str(self.jetpackFuel), 'plt:' + str(len(self.platforms)), 'bird:' + str(len(self.mobs)), 'cld:' + str(len(self.clouds)), 'acc:' + str(round(self.player.acc.x,2)), 'vel:' + str(round(self.player.vel.y,2))]
        pea_hits = pg.sprite.spritecollide(self.player, self.peabullets, False)
        if pea_hits:
            self.player.vel.y = random.randint(-5,-2)
            self.player.vel.x = random.randint(-3, 3)
    def hit(self):
        for sprite in self.all_sprites:
    ...
    def draw(self):
        ...
        if self.clock.get_fps() >= 58: 
            self.draw_text(str('FPS ' + str(round(self.clock.get_fps(), 1))), 20, LIME, 40, 4)
        if self.clock.get_fps() < 58:     
            self.draw_text(str('FPS ' + str(round(self.clock.get_fps(), 1))), 20, YELLOW, 40, 4)
        if self.clock.get_fps() <= 30:
            self.draw_text(str('FPS ' + str(round(self.clock.get_fps(), 1))), 20, RED, 40, 4) 
        if self.debugon == True:  
        ...
        if self.jetpackActive == True:
            ...
            self.draw_text('FUEL : ' + str(self.jetpackFuel), 20, self.color ,60, HEIGHT - 20)      
       # if self.jetpackActive == True:    
